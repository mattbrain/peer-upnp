...
Service.prototype.notify = function(){
	...
	for ( var sid in this.subscriptions) {
		...
		for ( var i = 0; i < callbacks.length; i++) {
			...
			console.log("Sending notification to: " +url.hostname + ":"+url.port+url.path +"Content: " + xml)
...
var subscribe = function(service){
	...
	console.log("->Subscribe Request:"+eventSubURL);
...
var renew = function(service){
	if (service.sid) {
		...
		var port = peer.server && peer.server.address().port;
		var eventSubURL = service.eventSubURL;
		console.log("->Renew Request:" + eventSubURL)
		var url = URL.parse(eventSubURL);
		var req = http.request({
			host: url.hostname,
			port: url.port,
...
var handleSubscribeEvent = function(req,rsp,peer){
	...
	if (sid != "") {
		console.log ("Sid is set in headers");
	}
	if (split.length==2) {
		var udn = split[0];
		var serviceType = split[1];
		var device = peer.devices[udn];
		var service = device && device.services[serviceType];
		var timeout = req.headers.timeout || "Second-60";
		var success = false;			
		console.log("Headers: "+ req.rawHeaders)
		console.log("UDN: " + udn + " ServiceType: "+ serviceType+ " Device: " + device + " Service: " + service)
		if (service) {
			if (sid == "") {
				// generate a new sid				
				sid = "uuid:"+UUID.v4();
				...
				console.log('Peer Subscribed' + callbacks);
				console.log('Notifying all subscriptions');
				setTimeout(function() {service.notify();},5000);
				success = true;
			} else if (sid in service.subscriptions) {
				service.subscriptions[sid]['timeout'] = timeout;
				console.log('Subscription Renewed' + service.subscriptions[sid]['timeout']);
				success = true;
			}
			if (success===true) {
				...
				// added expiry of subscriptions which are not renewed below
				timeout = timeout && parseInt(timeout.replace("Second-","")) || 1800;
				clearTimeout(service.subscriptions[sid]["timeoutHandle"]);
				service.subscriptions[sid]["timeoutHandle"] = setTimeout(function() {
					console.log ("Subscription Timed Out " + sid);
